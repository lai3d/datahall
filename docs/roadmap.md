...
| Social preview image and one-click screenshot | Spreading depends on links, and links need an image. One OG image; export the 3D view as PNG (`toDataURL` after `renderOnce` already works) | 1–2 h | Done, 2026-09-17 |
...
| "Learn more" panel | A paragraph and source link for each device type and each constraint; why the PUE coefficients are 0.08 and 0.30. Shipped as an in-app methodology dialog opened from the header and from links next to the sections; per-device source links wait for source metadata in the catalog (see the catalog data refresh) | 2–3 h | Done, 2026-09-18 |
| Catalog data refresh | Measured GB300 power after shipping, Rubin's official naming, Kyber figures re-checked, DGX B200 replaced by B300; add Huawei CloudMatrix 384 as a compute rack (32 Ascend 910C) plus an optical switch rack, the best contrast story to NVL72; TPU Ironwood only if a credible per-rack power figure appears. Before adding any air-cooled dense rack, turn the hardcoded `dgx` density warning in `sim.ts` into a catalog flag. Sources go into the `catalog.json` notes. Sources: every catalog figure now carries sources (official / reported / estimate) shown in the app, and values were re-checked on 2026-09-18 (GB200/GB300 prices, GB300 and Vera Rubin liquid share, Kyber renamed); DGX B200 kept so old share links keep their meaning ; CloudMatrix 384 added as one compute-rack type with the switch racks folded in, since the model has no scale-up constraint to check a separate switch rack against | 2 h | Done, 2026-09-18 |
