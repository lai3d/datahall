...
import {phasesIn, growthPlan, headroom, utilization} from '../src/growth.js';
import {PRESETS} from '../src/layout.js';
import {toItems} from '../src/edit.js';

const at = (type, x, z, phase) => phase ? {type, x, z, phase} : {type, x, z};
  ...
  it('算出的台数加上去仍满足总量，再多一台就不满足（每种 GPU 机柜、每个预设）', () => {
    ...
    for (const p of [PRESETS.gb200, PRESETS.gb200n1, PRESETS.rubin]){
      ...
      for (const type of gpuTypes){
        ...
        const add = n => [...base, ...Array.from({length: n}, (_, i) => at(type, i, 9))];
        const totalsOk = list => !compute(list, CAT, p.u).issues.some(i => i.lvl === 'bad');
...
describe('分享链接里的阶段', async () => {
  ...
  const {sameLayout} = await import('../src/edit.js');
  const {GRID} = await import('../src/grid.js');

  it('有阶段时用版本 3，解码还原；阶段 1 不写', () => {
    const p = {u: 5, list: [['vr200', 0, 3], ['vr200', 1, 3, {phase: 2}], ['cdu', 0, 5, {phase: 3}], ['vr200', 2, 3, {phase: 2, feeds: {coolantSource: [0, 5]}}]]};
...
describe('阶段贯穿 USD 和 layout.json', async () => {
  ...
  const {toEntry, sameLayout} = await import('../src/edit.js');
  const {GRID} = await import('../src/grid.js');
