...
import {readSample} from '../scripts/sample.ts';
...
describe('buildLayout', () => {
  ...
  it('sample: topology matches the relationships in the USD export exactly', () => {
    ...
    for (const e of layout.equipment){
      ...
      const rel = (name: string) => body.match(new RegExp(`rel dchall:${name} = </DataHall/Equipment/(\\w+)>`))?.[1] || '';
  ...
  it('catalog lists only the device types used, with parameters from catalog.json', () => {
    ...
    expect(layout.catalog[0]).toMatchObject({name: 'Vera Rubin NVL72', category: 'gpu', powerKw: 190, gpuCount: 72, liquidFraction: 1, heightM: 2.3, roadmap: false});
