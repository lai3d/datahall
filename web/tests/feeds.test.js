...
describe('手动指定贯穿各个格式', async () => {
  ...
  const {manualItems} = await import('../scripts/manual-feeds-usda.js');
  const items = manualItems();
  const snap = {u: 2, list: items.map(i => i.feeds ? [i.type, i.x, i.z, i.feeds] : [i.type, i.x, i.z])};
  const feedsOf = list => list.filter(e => e[3]).map(e => [e[1], e[2], e[3]]).sort((a, b) => a[0] - b[0]);
