...
//   feeds：手动指定的供给设备（grid.js 的 supplyLinks）；phase：部署阶段，大于 1 才写（growth.js）
// 2026-09-17 短暂发布过第 4 项直接是 feeds（{coolantSource, powerFeed}）的写法，localStorage 里可能还有，读的时候兼容
export function entryProps(entry){
  const p = entry[3];
  if (!p) return {};
  return 'coolantSource' in p || 'powerFeed' in p ? {feeds: p} : p;
}
...
export function toItem(entry){
  ...
  if (phase > 1) it.phase = phase;
  return it;
}
export const toItems = list => list.map(toItem);
// 设备对象 → 快照条目（深拷贝）
export function toEntry(it){
  const props = {};
  if (it.feeds) props.feeds = clone(it.feeds);
  if (it.phase > 1) props.phase = it.phase;
...
const canonical = p => JSON.stringify([p.u, p.list.map(e => JSON.stringify(toEntry(toItem(e)))).sort()]);
