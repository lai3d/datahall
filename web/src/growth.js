...
import {blockingReasons} from './redundancy.js';

export const phaseOf = it => it.phase || 1;

// 布局里出现过的阶段，升序；空布局返回 []
export const phasesIn = items => [...new Set(items.map(phaseOf))].sort((a, b) => a - b);

// 五项容量的利用率（需求 / 容量）。容量为 0 且有需求时记为 Infinity，都为 0 时记为 0
export function utilization(s, utility){
  const ratio = (need, cap) => cap > 0 ? need / cap : need > 0 ? Infinity : 0;
  return {
...
export function growthPlan(items, CAT, utility){
  return phasesIn(items).map(phase => {
    ...
    const [kind, ratio] = Object.entries(util).reduce((a, b) => b[1] > a[1] ? b : a);
...
export function headroom(items, CAT, utility, type){
  const s = compute(items, CAT, utility), t = CAT[type];
  const kw = t.kw || 0, liq = t.liq || 0;
  // 每加一台的需求增量，和 sim.js 的 PUE 公式一致：IT + 自耗 + 液冷热量×0.08 + 风冷热量×0.30 + IT×0.05
  const need = {
    dist: kw,
    liquid: kw * liq,
    air: kw * (1 - liq),
  ...
  let best = {limit: null, count: Infinity};
