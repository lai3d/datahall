...
export function zipStore(files: {name: string; text: string}[]): Blob{
  const enc = new TextEncoder(), parts: Uint8Array<ArrayBuffer>[] = [], central: Uint8Array<ArrayBuffer>[] = [];
  const now = new Date();
