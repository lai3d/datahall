...
- `web/`: web version, TypeScript (strict) + Vite + three.js 0.186 (version pinned in npm) + React 19 for the panel, entry `web/index.html` → `web/src/main.ts`
  ...
  - UI architecture: `main.ts` owns the app logic and mutates `state` (`state.ts`), then calls `notify()` (`store.ts`); `ui.tsx` renders the right-hand panel and the overlay on the 3D view (HUD, undo / redo, reset view, via a portal into `#stage`) with React 19,
    ...
    The 3D scene (`scene.ts`, `controls.ts`) stays plain three.js. Panel-only state such as share / import messages and export progress lives in `state.ui`.
    Below the builder (utility, devices, capacity check, details) the panel has three groups in a tablist (`#modes`, tabs `#tab-<mode>`, panels `#mode-<mode>`): Learn (scenarios, presets, rack comparison), Design (goal, growth plan, energy and ownership, back-end fabric) and Drill (failure drill and N+1); Share and the OpenUSD section stay visible below them. `state.ui.mode` is view state kept in localStorage `datahall.mode`, the hidden groups stay mounted (form drafts survive), a scenario with `panel` in `scenarios.ts` opens that group when it starts, and e2e tests open a group with `openMode` before touching its sections. Tutorial targets must stay in the always-visible part Keep the DOM ids and `data-*` attributes stable: the e2e smoke tests depend on them
  ...
  - `src/rack-faces.ts`: procedural device fronts for the 3D view, drawn on canvases (color, bump and glow maps), one set per device type and theme, cached and shared (`clearFaces` on a theme change). Racks with catalog `parts` are drawn from those counts (`partsStack`: NVL72 as power shelves, compute trays split around the NVLink switch trays, power shelves; servers, nodes and switches stacked; blank panels fill to 44U), so the picture matches the details panel's "Inside the rack" list (`ui.tsx`'s `Parts`, `#parts` with `data-parts`); the others (Kyber, Helios, storage) use illustrative `STACKS` and the panel says so. Never vendor artwork; the glow map marks status lights and accent light bars, and the front material is the one the power-on animation drives. CDUs and RPPs leave the right third of the front (`METER_X`) as a recess for the load meter
  - `src/designs.ts`: compare two designs, pure functions. `summarize` reduces a layout (every phase, no failure drill, like the report) to GPUs, racks by type, devices, power, PUE, feed, hardware, annual energy and cost at the energy inputs, the power-on problems and the single points of failure; `delta` marks a change better or worse only where the direction is clear (more GPUs, lower PUE). The pinned design (`state.ui.pinned`: layout, time, pin or link) is view state in localStorage `datahall.pinned`, read back through `parseSavedLayout`, never in share links or exports. UI `Designs` (`#designs`, Design tab): pin, swap (loads the pinned hall as one undo entry and pins the current one), unpin, and pin from a pasted share link (`decodeLayout`)
...
- **3D rendering** (three 0.186): color management is on by default; CSS colors are read as sRGB, converted to linear space for computation, and output as sRGB; no tone mapping, so palette colors do not shift.
  ...
  The render loop draws on demand: only when an exported `scene.ts` mutator set `dirty`, the camera matrix changed, or something animates (a placement pop, powered-on glow and flow; with reduced motion only the first 3 s after power-on). A new function that changes what the scene shows must set `dirty`
...
- Rack parts (`parts`, checked 2026-09-22): GB200 NVL72 18 × 1RU compute trays (2 Grace, 4 Blackwell), 9 × 1RU NVLink switch trays, 8 power shelves of 6 × 5.5 kW (DGX GB user guide); GB300 18 compute trays, 9 switch trays, 8 shelves of 6 × 5.5 kW (GB300 SuperPOD RA); Vera Rubin 18 compute trays, 9 switch trays, 4 shelves of 6 × 18.3 kW, 110 kW each (Vera Rubin RA). Heights appear only where a source gives them (`u`), so GB300 and Vera Rubin trays have none. DGX B200 4 × 10RU systems of 8 GPUs; MI355X 8 × 4U servers of 8 GPUs (GIGABYTE G4L3 datasheet; the count follows from 64 GPUs); CloudMatrix 4 nodes of 8 NPUs and 4 CPUs per compute rack (48 nodes over 12 racks, Huawei's arXiv paper); IB rack 2 × 4U Q3400 with 144 ports. Kyber and Helios have no published layout. `tests/catalog.test.ts` checks the parts add up to `gpus` and `ports`
